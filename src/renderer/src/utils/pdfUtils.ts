...
export async function loadPdf(
  ...
  try {
    ...
    worker.onerror = (err: any) => {
      ...
      logToTerminal(
        `[pdfUtils] Worker onerror fired: ${err.message || String(err)}`,
      );
    };
    ...
    logToTerminal(
      "[pdfUtils] Stage 1: Starting getDocument via injected Vite worker...",
    );
    const loadingTask =
    ...
    logToTerminal(
      `[pdfUtils] Stage 1: Awaiting promise with ${timeout}ms timeout...`,
    );
    ...
    if (verbose) {
      console.log(
        `[pdfUtils] ✓ PDF loaded successfully (${result ? result.numPages : 0} pages)`,
      );
      logToTerminal(
        `[pdfUtils] ✓ PDF loaded successfully (${result ? result.numPages : 0} pages)`,
      );
    ...
    logToTerminal(
      `[pdfUtils] Error in Stage 1: ${error instanceof Error ? error.message : String(error)}`,
    );
    throw new Error(
...
export async function renderPageToArrayBuffer(
  ...
  try {
    ...
    console.error(
      `[pdfUtils] Failed to render page ${pageNumber} to buffer:`,
      error,
    );
    throw new Error(
      `Failed to render page ${pageNumber} to buffer: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
